...
public class FixturesItemsProviderImplTest {

    @Test
    public void test_getItems_returnsListOfItemsWithHeaders() {
        ...
        Gson gson = new Gson();
        FixturesController fixturesController = mock(FixturesController.class);
        Fixture[] fixturesArray = gson.fromJson(fixturesString, FixtureImpl[].class);
        when(fixturesController.getFixtures()).thenReturn(Arrays.asList(fixturesArray));
        FixturesItemsProviderImpl fixturesItemsProvider = new FixturesItemsProviderImpl(fixturesController);

        List<Item> items = fixturesItemsProvider.getItems();
    }

    @Test
    public void test_formatDate_withValidDate_returnsExpectedValue() {
        FixturesController fixturesController = mock(FixturesController.class);
        FixturesItemsProviderImpl fixturesItemsProvider = new FixturesItemsProviderImpl(fixturesController);

        String formattedDate = fixturesItemsProvider.formatDate("2019-02-10T16:00:00.000Z");

        Assert.assertEquals("February 2019", formattedDate);
    }

    @Test
    public void test_formatDate_withInvalidNumericDate_returnsExpectedValue() {
        FixturesController fixturesController = mock(FixturesController.class);
        FixturesItemsProviderImpl fixturesItemsProvider = new FixturesItemsProviderImpl(fixturesController);

        String formattedDate = fixturesItemsProvider.formatDate("12345");

        Assert.assertEquals("N/A", formattedDate);
    }

    @Test
    public void test_formatDate_withInvalidAlphanumericDate_returnsExpectedValue() {
        FixturesController fixturesController = mock(FixturesController.class);
        FixturesItemsProviderImpl fixturesItemsProvider = new FixturesItemsProviderImpl(fixturesController);

        String formattedDate = fixturesItemsProvider.formatDate("asdfg");

        Assert.assertEquals("N/A", formattedDate);
    }

    @Test
    public void test_formatDate_withNullDate_returnsExpectedValue() {
        FixturesController fixturesController = mock(FixturesController.class);
        FixturesItemsProviderImpl fixturesItemsProvider = new FixturesItemsProviderImpl(fixturesController);

        String formattedDate = fixturesItemsProvider.formatDate(null);

        Assert.assertEquals("N/A", formattedDate);
    }

    @Test
    public void test_formatDate_withEmptyDate_returnsExpectedValue() {
        FixturesController fixturesController = mock(FixturesController.class);
        FixturesItemsProviderImpl fixturesItemsProvider = new FixturesItemsProviderImpl(fixturesController);

        String formattedDate = fixturesItemsProvider.formatDate("");
